...
static const char raw_0_padding[] =
...
static const char enc_0_padding[] =
...
void test_encoding(void)
{
    ...
    len = base64_encode(&buff, &raw_0_data, strlen(raw_0_data));
    assert(len == strlen(enc_0_data));

    len = base64_encode(&buff, &raw_0_padding, strlen(raw_0_padding));
    assert(len == strlen(enc_0_padding));
    assert(memcmp(&buff, &enc_0_padding, len) == 0);

    len = base64_encode(&buff, &raw_1_padding, strlen(raw_1_padding));
    assert(len == strlen(enc_1_padding));
    assert(memcmp(&buff, &enc_1_padding, len) == 0);

    len = base64_encode(&buff, &raw_2_padding, strlen(raw_2_padding));
    assert(len == strlen(enc_2_padding));
    assert(memcmp(&buff, &enc_2_padding, len) == 0);

    printf("<<< PASS\n");
}

static const char enc_invalid_len[] =
    "SW4gYm90aCBjaGFwdGVycyAxNTIgYW5kIDE2Miwgd2Ugc2VlIHRoYXQgZHVyaW5nIHdpbnRlci"
    "BicmVhaw";

static const char enc_invalid_char_in_data[] =
    "SW4gYm90aCBjaG*wdGVycyAxNTIgYW5kIDE2Miwgd2Ugc2VlIHRoYXQgZHVyaW5nIHdpbnRlci"
    "BicmVhaw==";

static const char enc_invalid_char_in_padding[] =
    "SW4gYm90aCBjaGFwdGVycyAxNTIgYW5kIDE2Miwgd2Ugc2VlIHRoYXQgZHVyaW5nIHdpbnRlci"
    "BicmVhaw=&";

static const char enc_too_many_padding_char[] =
    "SW4gYm90aCBjaGFwdGVycyAxNTIgYW5kIDE2Miwgd2Ugc2VlIHRoYXQgZHVyaW5nIHdpbnRlci"
    "BicmVhaw======";

static const char enc_invalid_padding_char_position[] =
...
void test_decoding(void)
{
    ...
    ret = base64_decode(&buff, &enc_invalid_char_in_data,
                        strlen(enc_invalid_char_in_data));
    assert(ret == BASE64_ERR_INVALID_ENC_CHAR);
    assert(base64_error_param() == 14);

    ret = base64_decode(&buff, &enc_invalid_char_in_padding,
                        strlen(enc_invalid_char_in_padding));
    assert(ret == BASE64_ERR_INVALID_ENC_CHAR);
    assert(base64_error_param() == 83);

    ret = base64_decode(&buff, &enc_too_many_padding_char,
                        strlen(enc_too_many_padding_char));
    assert(ret == BASE64_ERR_INVALID_ENC_PADDING);
    assert(base64_error_param() == 82);

    ret = base64_decode(&buff, &enc_invalid_padding_char_position,
                        strlen(enc_invalid_padding_char_position));
    assert(ret == BASE64_ERR_INVALID_ENC_PADDING);

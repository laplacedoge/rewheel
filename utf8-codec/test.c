...
static const char encoded_data_only_greek_alphabet[] = "ΑαΒβΓγΔδΕεΖζΗηΘθΙιΚκΛλΜμΝνΞξΟοΠπΡρΣσςΤτΥυΦφΧχΨψΩω";
...
static const char encoded_data_triple_mixed[] = "\t<p>是的，first let's 设置 β to 1.9, 然后静观其变！</p>";
static const uint16_t decoded_data_triple_mixed[] =
{
...
void test_decode_right_encoded_data(void)
{
    ...
    ret = utf8_decode(code_points, encoded_data_only_ascii, sizeof(encoded_data_only_ascii) - 1, &decoded_num, NULL, 0);
    assert(ret == UTF8DEC_OK);
    assert(decoded_num == 18);
    assert(memcmp(code_points, decoded_data_only_ascii, 18) == 0);

    ret = utf8_decode(code_points, encoded_data_only_greek_alphabet, sizeof(encoded_data_only_greek_alphabet) - 1, &decoded_num, NULL, 0);
    assert(ret == UTF8DEC_OK);
    assert(decoded_num == 49);
    assert(memcmp(code_points, decoded_data_only_greek_alphabet, 49) == 0);

    ret = utf8_decode(code_points, encoded_data_only_chinese, sizeof(encoded_data_only_chinese) - 1, &decoded_num, NULL, 0);
    assert(ret == UTF8DEC_OK);
    assert(decoded_num == 12);
    assert(memcmp(code_points, decoded_data_only_chinese, 12) == 0);

    ret = utf8_decode(code_points, encoded_data_triple_mixed, sizeof(encoded_data_triple_mixed) - 1, &decoded_num, NULL, 0);
    assert(ret == UTF8DEC_OK);
    assert(decoded_num == 43);
    assert(memcmp(code_points, decoded_data_triple_mixed, 43) == 0);
